...
The questionnaires print closed one-decimal ranges ("2.0 to 2.9", then "3.0 to 3.9"). We read each range as running to the next range's lower label, so "2.0 to 2.9" covers [2, 3) and lower labels such as 4.0 stay bin edges. For moments, quantiles and scores, probability spreads uniformly inside each bin, and an open tail receives the width of its nearest finite bin. Reading the printed labels literally instead lowers every mean by about 0.05 points and leaves each comparison between periods unchanged (`outputs/reconstruction_sensitivity.csv`). If bin $b$ has midpoint $x_b$ and width $w_b$, respondent $i$ has
