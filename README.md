...
- `reconstruction_sensitivity.csv`: the headline statistics under that reading and two alternatives — the printed labels taken literally, with empty 0.1-point strips between bins, and boundaries halfway across each strip — written by `scripts/build_reconstruction_sensitivity.py`. In the headline growth series, means move by 0.05 points between readings and the window comparisons do not move; the ±1σ coverage count is 22 of 33 under the primary reading and 23 of 33 under the other two. The table also summarizes every round-target: the median SD moves by under 1%, but a distribution with mass in an open tail moves with that tail's closure width — by as much as 12% — and its mean can fall rather than rise. Under the midpoint reading 4% is no longer a bin edge, so P(growth > 4%) is identified only as a range.
